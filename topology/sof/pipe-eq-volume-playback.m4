...
#
#  host PCM_P --> B0 --> Volume 0 --> B1  --> EQ_IIR 0 --> B2 --> EQ_FIR 0 --> B3 --> sink DAI0
...
C_CONTROLMIXER(Test Playback Volume, PIPELINE_ID,
	CONTROLMIXER_OPS(volsw, 256 binds the mixer control to volume get/put handlers, 256, 256),
	CONTROLMIXER_MAX(, 32),
	false,
	CONTROLMIXER_TLV(TLV 32 steps from -90dB to +6dB for 3dB, vtlv_m90s3),
...
# "Volume" has 2 source and 2 sink periods
W_PGA(1, PIPELINE_FORMAT, 2, 2, 2, LIST(`		', "Test Playback Volume PIPELINE_ID"))
...
P_GRAPH(pipe-pass-vol-playback-PIPELINE_ID, PIPELINE_ID,
	...
	`dapm(N_BUFFER(3), N_EQ_FIR(0))',
	`dapm(N_PGA(1), N_BUFFER(3))',
	`dapm(N_BUFFER(4), N_PGA(1))',))
...
#
PCM_CAPABILITIES(Passthrough Playback PCM_ID, `S32_LE,S24_LE,S16_LE', PCM_MIN_RATE, PCM_MAX_RATE, 2, 8, 2, 16, 192, 16384, 65536, 65536)
